...
namespace aes
{
	...
	void key_expansion_aes(uint8_t* key, int& key_size, uint32_t* w, int& w_size)
	{
		std::vector<uint8_t> key_vec(key, key + key_size);
		std::vector<uint32_t> w_vec;
		key_expansion(key_vec, w_vec);
		std::copy(key_vec.begin(), key_vec.end(), key);
		std::copy(w_vec.begin(), w_vec.end(), w);
		w_size = w_vec.size();
	}

	void cipher_aes(uint8_t* in, uint8_t* out, uint32_t* dw, int& dw_size)
	{
		std::vector<uint8_t> in_vec(in, in + 16), out_vec(out, out + 16);
		std::vector<uint32_t> dw_vec(dw, dw + dw_size);
		cipher(in_vec, out_vec, dw_vec);
		std::copy(out_vec.begin(), out_vec.end(), out);
		std::copy(dw_vec.begin(), dw_vec.end(), dw);
	}

	void inv_cipher_aes(uint8_t* in, uint8_t* out, uint32_t* dw, int& dw_size)
	{
		std::vector<uint8_t> in_vec(in, in + 16), out_vec(out, out + 16);
		std::vector<uint32_t> dw_vec(dw, dw + dw_size);
