#include "pch.h"
#include "Kalyna.h"
#include "Kalyna_lookup_tables.h"
...
namespace kalyna
{
	...
	void key_expansion_kalyna(uint8_t* key, uint64_t** w)
	{
		std::vector<uint8_t> key_vec(key, key + key_size);
		std::vector<std::vector<uint64_t>> w_vec;
		for (int i = 0; i < nr + 1; ++i)
		{
			w_vec.push_back(std::vector<uint64_t>(w[i], w[i] + nb));
		}
		key_expansion(key_vec, w_vec);
		std::copy(key_vec.begin(), key_vec.end(), key);
		for (int i = 0; i < nr + 1; ++i)
		{
			std::copy(w_vec[i].begin(), w_vec[i].end(), w[i]);
		}
	}

	void cipher_kalyna(uint8_t* in, uint8_t* out, uint64_t** dw)
	{
		std::vector<uint8_t> in_vec(in, in + 16), out_vec(out, out + 16);
		std::vector<std::vector<uint64_t>> dw_vec;
		for (int i = 0; i < nr + 1; ++i)
		{
			dw_vec.push_back(std::vector<uint64_t>(dw[i], dw[i] + nb));
		}

		cipher(in_vec, out_vec, dw_vec);

		std::copy(in_vec.begin(), in_vec.end(), in);
		std::copy(out_vec.begin(), out_vec.end(), out);
		for (int i = 0; i < nr + 1; ++i)
		{
			std::copy(dw_vec[i].begin(), dw_vec[i].end(), dw[i]);
		}
	}

	void inv_cipher_kalyna(uint8_t* in, uint8_t* out, uint64_t** dw)
	{
		std::vector<uint8_t> in_vec(in, in + 16), out_vec(out, out + 16);
		std::vector<std::vector<uint64_t>> dw_vec;
		for (int i = 0; i < nr + 1; ++i)
		...
		inv_cipher(in_vec, out_vec, dw_vec);
